...
you realize that the Elves are already loading you into a trebuchet ("please hold still, we need to
strap you in").
...
* */
pub fn solution() {
    let calibration_string = fs::read_to_string("inputs/day1.txt").unwrap();
    let mut calibration_sum = 0;

    calibration_string.lines().for_each(|line| {
        let mut line_num = String::new();
        
        line_num.push(line.chars().find(|ch| ch.is_ascii_digit()).unwrap_or('0'));
        line_num.push(line.chars().rev().find(|ch| ch.is_ascii_digit()).unwrap_or('0'));
